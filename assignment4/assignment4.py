...
def make_engine():
    """MariaDB engine creation"""
    my_db = URL.create(
        drivername="mariadb",
        query={"read_default_file": os.path.expanduser("~/.my.cnf")},
    )
    engine = create_engine(my_db, echo=False)
    return engine


def create_species_table(
    engine,
):  # Species name, Species acc, Genome size, Number of genes, Number of proteins, TaxDB
    """Create table with species columns"""
    metadata = MetaData()
    species = Table(
        "species",
        metadata,
...
def create_protein_table(
    engine,
):  # Protein ID, Protein Product, Protein Location, Locus Tag, Gene ref, EC number, GO Annotation
    """Create table with protein columns"""
    metadata = MetaData()
    proteins = Table(
        "proteins",
        metadata,
...
def create_link_table(
    ...
    """Create table linking proteins to species"""
    metadata = MetaData()
...
def process_batch(connection, table, insertion_list):
    """Insert list of dictionaries into specified table"""
    connection.execute(table.insert(), insertion_list)


def parse_genbank(
    ref_path, engine, species_table, protein_table, link_table, batch_size=5000
):
    ...
    protein_buffer = (
        []
    )  # Protein ID, Protein Product, Protein Location, Locus Tag, Gene ref, EC numb, GO Annotation
    link_buffer = []
    ...
    for record in SeqIO.parse(ref_path, format="genbank"):
        ...
        for feature in features:
            ...
            if ftype == "source":
                ...
                link_dict = {"protein_id": protein_id, "species_id": record_id}
                protein_dict = {
...
def main(args):
    ...
    parse_genbank(
        args.input, engine, species_table, protein_table, link_table, batch_size=5000
    )
    # FINISH
    return 0
